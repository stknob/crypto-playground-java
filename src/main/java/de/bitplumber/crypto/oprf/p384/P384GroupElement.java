...
public class P384GroupElement implements GroupElement {
	...
	static {
		...
		Z = curve.fromBigInteger(BigInteger.valueOf(-12).mod(q));
		A = curve.getA();
		B = curve.getB();
	}
	...
	private static ECPoint hash_to_curve(byte[] input, byte[] DST) {
		final var u = hash_to_field(input, DST, 1, 192, 2);
	...
	public static P384GroupElement hashToGroup(byte[] input, byte[] DST) {
