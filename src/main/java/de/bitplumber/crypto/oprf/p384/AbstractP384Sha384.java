...
public abstract class AbstractP384Sha384 {
	...
	protected P384GroupElement hashToGroup(byte[] hash, byte[] customDST) {
		final var dst = ObjectUtils.defaultIfNull(customDST, Arrays.concatenate(Labels.HASH_TO_GROUP, context()));
		return P384GroupElement.hashToGroup(hash, dst);
	}

	protected P384FieldElement hashToScalar(byte[] hash, byte[] customDST) {
		final var dst = ObjectUtils.defaultIfNull(customDST, Arrays.concatenate(Labels.HASH_TO_SCALAR, context()));
		final var uniformBytes = expandMessageXMD(hash, dst, 72);
