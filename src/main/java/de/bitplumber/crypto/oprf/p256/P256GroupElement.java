...
public class P256GroupElement implements GroupElement {
	...
	static {
		...
		Z = curve.fromBigInteger(BigInteger.valueOf(-10).mod(q));
		A = curve.getA();
		B = curve.getB();
	}
	...
	// private static ECFieldElement[] hash_to_field_m_eq_1(byte[] input, byte[] DST, int k, int count) {
	// 	final var L = Math.ceilDiv(curve.getFieldSize() + k, 8);
	// 	final var lengthInBytes = count * L;
	// 	final var uniformBytes = expand_message_xmd(input, DST, lengthInBytes);
	// 	final var u = new ECFieldElement[count];
	// 	for (int i = 0; i < count; i++) {
	// 		final var elmOffset = L * i;
	// 		final var tv = Arrays.copyOfRange(uniformBytes, elmOffset, elmOffset + L);
	// 		u[i] = curve.fromBigInteger(BigIntegers.fromUnsignedByteArray(tv).mod(q));
	// 	}
	// 	return u;
	// }

	private static ECPoint hash_to_curve(byte[] input, byte[] DST) {
		final var u = hash_to_field(input, DST, 1, 128, 2);
	...
	public static P256GroupElement hashToGroup(byte[] input, byte[] DST) {
