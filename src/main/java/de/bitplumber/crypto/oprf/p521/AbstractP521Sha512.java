...
public abstract class AbstractP521Sha512 {
	...
	public P521GroupElement decodeElement(byte[] input) throws Exception {
		return P521GroupElement.fromBytes(input);
	}

	public byte[] encodeScalar(P521FieldElement scalar) {
		return scalar.toByteArray();
	}

	public P521FieldElement decodeScalar(byte[] input) throws Exception {
	...
	public P521FieldElement randomScalar() {
		final var uniformBytes = RandomUtils.secureStrong().randomBytes(98);
	...
	public KeyPair randomKeypair() {
		...
		final var publicElement = P521GroupElement.BASEPOINT.multiply(secretScalar);
	...
	public KeyPair deriveKeypair(byte[] seed, byte[] info) throws Exception {
		...
		int counter = 0;
		P521FieldElement secretScalar = P521FieldElement.ZERO;
		while (P521FieldElement.ZERO.ctEquals(secretScalar)) {
		...
		final var publicKey = P521GroupElement.BASEPOINT.multiply(secretScalar).toByteArray();
	...
	protected static final record CompositesResult(P521GroupElement M, P521GroupElement Z) {}

	protected CompositesResult computeCompositesFast(P521FieldElement k, P521GroupElement B, P521GroupElement[] C, P521GroupElement[] D) {
		...
		var M = P521GroupElement.IDENTITY;
	...
	protected CompositesResult computeComposites(P521GroupElement B, P521GroupElement[] C, P521GroupElement[] D) {
		final var bm = encodeElement(B);
		final var seedDST = Arrays.concatenate(Labels.SEED_DST_PREFIX, context());
		final var seed = hash(Arrays.concatenate(
			I2OSP(bm.length, 2), bm,
		...
		var M = P521GroupElement.IDENTITY;
		var Z = P521GroupElement.IDENTITY;
		for (var i = 0; i < C.length; i++) {
			final var Ci = encodeElement(C[i]);
			final var Di = encodeElement(D[i]);
			final var di = hashToScalar(Arrays.concatenate(new byte[][]{
	...
	protected Proof generateProof(P521FieldElement k, P521GroupElement A, P521GroupElement B, P521GroupElement[] C, P521GroupElement[] D, P521FieldElement proofRandomScalar) {
	...
	protected Proof generateProof(P521FieldElement k, P521GroupElement A, P521GroupElement B, P521GroupElement[] C, P521GroupElement[] D) {
		return generateProof(k, A, B, C, D, null);
	}

	protected boolean verifyProof(P521GroupElement A, P521GroupElement B, P521GroupElement[] C, P521GroupElement[] D, Proof proof) throws Exception {
